  ...
  return (
    <dialog id="send_algo_modal" className={`modal ${openModal ? 'modal-open' : ''}`}>
      <form method="dialog" className="modal-box">
        <h3 className="font-headline font-bold text-2xl text-on-surface mb-1">Send Algo</h3>
        <p className="text-on-surface-variant text-sm mb-5">Send a payment transaction to any address</p>
        <div className="flex flex-col gap-3">
          <input className="input input-bordered w-full" placeholder="Recipient address" value={to} onChange={(e) => setTo(e.target.value)} />
          <input className="input input-bordered w-full" placeholder="Amount (ALGO)" value={amount} onChange={(e) => setAmount(e.target.value)} />
        </div>
        <div className="modal-action">
          <button className={`btn bg-accent-green text-surface-base font-bold hover:bg-accent-green-dim ${loading ? 'loading' : ''}`} onClick={onSend} disabled={loading}>Send</button>
