  ...
  const onMint = async () => {
    ...
    try {
      ...
      const metadata = {
        ...
        properties: { simple_property: 'Dashing Item' },
      }
  ...
  return (
    <dialog id="mint_nft_modal" className={`modal ${openModal ? 'modal-open' : ''}`}>
      <form method="dialog" className="modal-box">
        <h3 className="font-headline font-bold text-2xl text-on-surface mb-1">Mint NFT (ARC-3)</h3>
        <p className="text-on-surface-variant text-sm mb-5">Upload to IPFS and mint on Algorand</p>
